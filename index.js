...
const categories = {
    ...
    breaks:{
        options: [
            {value: `locking`, text: `Brake Locking`},
            {value: `unstable`, text: `Brake Unstable`},
            {value: `improve_breaking_performance`, text: `Improve Braking Performance`},
        ]
    },
    race_conditions:{
        options:[
            ...
            {value: `race_condition_breaks`, text: `Brakes`},
        ]
    },
    other:{
        options:[
            {value: `generally_unstable`, text: `Generally Unstable`},
            {value: `excessive_scraping_bottoming_out`, text: `Excessive Scraping/Bottoming Out`},
    ...
    race_condition_breaks:{
        options:[
            {value: `excessive_break_wear`, text: `Excessive Brake Wear`},
            {value: `excessive_break_temperature`, text: `Excessive Brake Temperature`},
        ]
    },
    ...
    corner_entry_understeer: {
        solutions: {
            tyres: [
                ...
                `More Toe Out <small style=\"color: ${sidenote_color}\">(negative decrease)</small>`,
                `More Front Camber <small style=\"color: ${sidenote_color}\">(negative increase)</small>`,
                `More Caster`
            ],
            mechanical_grip: [
                `Less Front Anti-roll Bar <small style=\"color: ${sidenote_color}\">OR</small> more Rear Anti-roll Bar`,
...
function createSelectElement(categoryKey, selectId = "secondary_selections") {
    ...
    if (!categoryData || !categoryData.options) {
        ...
        return '<p class="status-message error-message">Error: configuration issue for this selection.</p>';
    }

    let selectHTML = `<label class="field" for="${selectId}">
        <span>${valuetotext(categoryKey)} detail</span>
        <select id="${selectId}" class="setup-select">
...
function showElement(element) {
    ...
    element.style.animation = "fadeIn 0.42s ease-out both";
}
...
primaryslct.addEventListener('change', function () {
    const value = primaryslct.value;
    second_container.innerHTML = '';
    solution_container.innerHTML = '';
    hideElement(solution_container);
    flowchart = [];

    if (value === "null") {
        hideElement(second_container);
        resetbtn.hidden = true;
        primaryslct.hidden = false;
        return;
    }

    flowchart.push(primaryslct.options[primaryslct.selectedIndex].text);

    if (categories[value] && categories[value].options) {
        second_container.innerHTML = createSelectElement(value, "secondary_selection_1");
        showElement(second_container);
        primaryslct.hidden = true;
        resetbtn.hidden = false;
    } else if (categories[value] && categories[value].solutions) {
        ...
        showElement(solution_container);
        primaryslct.hidden = true;
        hideElement(second_container);
        resetbtn.hidden = false;
    } else {
        second_container.innerHTML = '<p class="status-message warning-message">No further options or solutions are available for this category yet.</p>';
        showElement(second_container);
        primaryslct.hidden = true;
        resetbtn.hidden = false;
    }
...
second_container.addEventListener('change', function (event) {
    if (event.target && event.target.tagName === 'SELECT') {
        ...
        if (categories[value] && categories[value].solutions) {
            ...
            second_container.innerHTML = createSelectElement(value, nextSelectId);
            showElement(second_container);
            resetbtn.hidden = false;
        } else {
            solution_container.innerHTML = `<p class="flowchart-path">${createflow(flowchart)}</p><p class="status-message warning-message">No specific solutions are defined for this path yet.</p>`;
            showElement(solution_container);
            hideElement(second_container);
            resetbtn.hidden = false;
        }
...
function valuetotext(value) {
    ...
    return converted
        .replace(/\bBreaks\b/g, 'Brakes')
        .replace(/\bBreak\b/g, 'Brake')
        .replace(/\bBreaking\b/g, 'Braking')
        .replace(/Tempatures/g, 'Temperatures');
}

function createflow(flowchartArray) {
    return flowchartArray
        .map(step => `<span class="flow-step">${step}</span>`)
        .join(` <span class="flow-arrow" aria-hidden="true">&raquo;</span> `);
}

function printSolutions(value) {
    const categorySolutions = categories[value]['solutions'];
    if (!categorySolutions) return '<p class="status-message warning-message">No solutions available for this selection.</p>';
    ...
    let output = `<div class="solutions-grid">`;

    for (let categoryNameKey in categorySolutions) {
        const iconSrc = iconMap[categoryNameKey];
        const iconHTML = iconSrc ? `<img src="${iconSrc}" alt="${valuetotext(categoryNameKey)} icon" class="solution-category-icon">` : '';

        output += `<article class="solution-category-item">
                    <h3>
                        ${iconHTML}
                        <span>${valuetotext(categoryNameKey)}</span>
                    </h3>
